...
Binary stability constraints extend the capabilities of ABC systems with interactions that are either allowed or forbidden, akin to a binary switch. These constraints govern whether two or more elements in the system can form stable compounds or patterns, based on specific matching criteria. This matching can be visualized using a puzzle piece analogy: two pieces either fit together (stable) or not. The simplest form of binary stability involves pairwise interactions, such as a particle and its antiparticle, denoted by \( A \) and \( \overline{A} \), respectively, which usually results in annihilation. Multi-site binary stability generalizes this concept to scenarios where stability arises from the simultaneous matching of multiple sites, as seen in codon-anticodon matches in ribosomes or crystallization, where the arrangement of atoms depends on direction and position matches, mimicking the puzzle piece analogy. Such constraints ensure that only specific configurations form, driving the system toward ordered, low-entropy states. Binary stability constraints expand the computational capabilities of ABC systems by enabling the evolution of logic operations and decoders. 

For example, the XOR operation, fundamental to digital logic, can evolve from a random binary sequence  like \( 0110111000\dots \) where local interactions occur between adjacent pairs, such that pairs that are both 1 or both 0 are annihilated (stability=0). The result will be a sequence of alternating \( 0, 1 \) satisfying the XOR condition, and reducing the entropy of the sequence from 1 bit to 0 bits, since the resulting sequence is fully deterministic.

Applications of binary stability constraints in ABC systems are numerous. They can model cellular automata (CA) and fractals such as the Sierpiński triangle, implemented with Wolfram's Rule 90 \cite{wolfram1983statistical} as the xor of the left and right neighbor: 
...
Binary stability constraints highlight the duality of simplicity and complexity: local binary decisions underpin global computational capabilities. From modeling abiotic processes like crystallization to evolving logic decoders and fractals, these constraints provide a versatile framework for exploring the intersection of physics, computation, and emergent complexity.
...
A Catalyst \( C \) facilitating the reaction \( A + B \to AB \) can be interpreted as a conditional statement or a logical gate. The presence of \( C \) determines whether the reaction occurs, making it analogous to the logical "if" condition: "If \( C \), then \( A + B \to AB \)."
...
This can be encoded in an ABC system by associating the states of cells with patterns and defining stability imbalances to favor interactions that generate the correct next state. For example: we define \( (A, B, C) \) as the current states of a triplet of \( (left, center, right) \) cells, respectively. Then, use stability imbalances to prioritize outcomes matching the rule:
    \[
    S(A, B) = 0.9 \text{ if } A = 1, B = 1; \quad S(B, C) = 0.7 \text{ if } B = 1, C = 0.
    \]

We initialize the ABC system with random \( 0,1 \) patterns corresponding to the initial state of the CA and allow it to evolve according to the stability-driven dynamics. Mixed triplets with \( A, B \), like \( (A, B, A) \) and \( (B, A, B) \) map to a \( B \), reflecting the dominant stability of mixed \( A, B \), while other states map to an \( A \). The emergent dynamics reflect the behavior of Rule 30, with complex patterns arising over successive generations. This ability of ABC systems to replicate CA behavior highlights their potential for emergent computation and complex pattern formation.

\begin{figure}[htp]
    \centering
    \includegraphics[width=8cm,height=5cm]{ca_1}
    \caption{ABC System Evolution Mimicking Rule 30}
    \label{fig:ca_1}
\end{figure}


\section{Fractals and Agent-Based Models in ABC Systems}

Fractals are mathematical structures characterized by self-similarity, where patterns repeat at different scales. Traditionally, Cellular Automata (CA) have been used to generate fractals, such as the Sierpinski triangle, through deterministic rules. ABC systems provide an alternative, enabling the simulation of fractals through probabilistic interactions and stability-driven dynamics. This approach abstracts away deterministic constraints and introduces a more general framework for fractal generation.

In this method, the vertices of a triangle are represented as base elements (\(A\), \(B\), \(C\)) in an ABC system. Patterns evolve by selecting a vertex based on stability-driven probabilities and moving halfway toward it. Stability constraints guide the likelihood of interaction with each vertex, mimicking attractive forces or biases in natural systems. Over successive iterations, these interactions generate a pattern approximating the Sierpinski triangle.

Figure~\ref{fig:fractals} shows the emergent pattern produced by the ABC system using this approach. While the structure deviates slightly from the mathematical Sierpinski triangle due to the probabilistic nature of interactions, it demonstrates the capacity of ABC systems to approximate fractal-like behavior through generalized rules. This highlights the flexibility of ABC systems in modeling dynamic and emergent structures beyond deterministic frameworks.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.6\textwidth,height=150pt]{sierpinski_simulated.png}
    \caption{Emergent fractal-like pattern generated by an ABC system, approximating the Sierpinski triangle through probabilistic, stability-driven interactions.}
    \label{fig:fractals}
\end{figure}
