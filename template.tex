...
A simple 2-bit decode maps \(00 \to 0, 01 \to 1, 10 \to 2, 11 \to 3\). Consider two populations: \( P_A \), containing a large set of triplet symbols like \( 00[1],00[2],01[2],01[4]\dots \) (2 binary inputs followed by an integer output in brackets). The second population \( P_T \) contains only the 4 stable triplets: \( 00[0], 01[1], 10[2], 11[3] \), which evolved separately to be the target decoded sequences.

Now, mix the populations, and let cross-population elements interact only when triplet values match. When such interactions occurs, patterns in \( P_A \) with wrong outputs like \(01[2]\) will not match any triplet in \( P_T \) and get eliminated from the population, while patterns like \(01[1]\) will match and survive. So \( P_T \) acts as a template that eliminates all invalid values from \( P_A \). This effectively allows a small or singular population \( P_T \) that may have only evolved once, to replicate itself to a much larger population \( P_A \).
