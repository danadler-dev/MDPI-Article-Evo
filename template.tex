...
ABC systems can evolve computational functionality such as decoders through stability-driven selection. A simple 2-bit decoder maps \(00 \to [A], 01 \to [B], 10 \to [C], 11 \to [D]\). One reason a mapping may evolve is if A, B, C, D are powerful catalysts. A chemical example may be Platinum, Palladium,
Iron and Nickel. Such patterns may be rewarded with high stability values (\( S=0.9 \)), and persist longer in the
...
Denote the above population \( P_T \). It contains only the 4 evolved triplets: 
00[A], 01[B], 10[C], 11[D]. Now consider another much larger population \( P_A \), containing both correct and incorrect mapping triplets like  00[A], 00[B], 01[F]....). To replicate \( P_T \) to \( P_A \): mix the populations, and let cross-population elements interact only when the
